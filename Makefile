...
.PHONY: env
env: service
	$(OUT_DIR)/$(APP)_linux_amd64 --help | grep -o '$$[^ ]*'
...
linux64:
	...
	@GOOS=linux GOARCH=amd64 go build -o $(OUT_DIR)/$(APP)_linux_amd64 $(APP_MAIN)
.PHONY: linux32
linux32:
	@echo BUILDING $(OUT_DIR)/$(APP)_linux_386
	@GOOS=linux GOARCH=386 go build -o $(OUT_DIR)/$(APP)_linux_386 $(APP_MAIN)
.PHONY: windows64	
windows64:
	@echo BUILDING $(OUT_DIR)/$(APP)_windows_amd64
	@GOOS=windows GOARCH=amd64 go build -o $(OUT_DIR)/$(APP)_windows_amd64 $(APP_MAIN)
.PHONY: windows32
windows32:
	@echo BUILDING $(OUT_DIR)/$(APP)_windows_386
	@GOOS=windows GOARCH=386 go build -o $(OUT_DIR)/$(APP)_windows_386 $(APP_MAIN)
.PHONY: freebsd64
freebsd64:
	@echo BUILDING $(OUT_DIR)/$(APP)_freebsd_amd64
	@GOOS=freebsd GOARCH=amd64 go build -o $(OUT_DIR)/$(APP)_freebsd_amd64 $(APP_MAIN)
.PHONY: freebsd32
freebsd32:
	@echo BUILDING $(OUT_DIR)/$(APP)_freebsd_386
	@GOOS=freebsd GOARCH=386 go build -o $(OUT_DIR)/$(APP)_freebsd_386 $(APP_MAIN)
.PHONY: darwin64
darwin64:
	@echo BUILDING $(OUT_DIR)/$(APP)_darwin_amd64
	@GOOS=darwin GOARCH=amd64 go build -o $(OUT_DIR)/$(APP)_darwin_amd64 $(APP_MAIN)
